...
├── targetPlatform      目标平台预设（抖音 / B站 / 快手 / 视频号 / YouTube / 小红书 / 通用）
│                       ✅ 已实现（2026-04）  7 个平台，前端 ConfigStage 可选
├── colorGradePreset    全局色调基调预设（25 种，含暖黄古风 / 赛博霓虹 / 粉彩治愈 等）
│                       ✅ 已实现（2026-04）
│                       → 阶段 3：color keywords 注入角色图 / 场景图 prompt
│                       → 阶段 6：color keywords 注入分镜图 prompt
│                       → 冲突处理：当摄影师为该帧指定 color_tone 时，自动降级为
│                         仅保留质感词（去除具体色相词），scene color_tone 作为最终色彩指令
│                         追加到 style 末尾，避免两条「必须严格遵守」互相打架
└── projectTemplate     项目模板（25 种，含古装言情 / 都市甜宠 / 悬疑犯罪 等）
                        ✅ 已实现（2026-04）  一键填充 artStyle + colorGradePreset +
                          targetPlatform + contentTrack 组合配置
```
...
| 项目 | 内容 |
|------|------|
| **已完成** | ✅ 目标平台预配置（targetPlatform，7 平台）；✅ 全局色调基调（colorGradePreset，25 预设，阶段 3+6 全链路注入，含色相冲突自动降级）；✅ 项目模板库（25 模板，一键组合配置） |
| **缺失功能** | 内容风格基调锁定；模板自定义保存；平台导出尺寸与 videoRatio 联动自动写入 |
| **优先级** | 🟡 中 |
| **待实现** | ① **平台→比例联动**：选择目标平台后自动填写 videoRatio（抖音→9:16 / B站→16:9 / YouTube 横屏→16:9），减少用户手动配置步骤 ② **模板自定义保存**：支持用户将当前配置组合另存为自定义模板，在下次新建项目时直接复用 ③ **内容风格基调锁定**：在 contentTrack 之外新增"基调约束"字段（如"严禁血腥""仅限轻松向"），后续 LLM 生成阶段（剧本/分镜）自动在 system prompt 注入此约束 |
| **实现好处** | 平台联动减少配置失误；自定义模板支撑批量项目生产；基调约束从源头保障内容安全 |
