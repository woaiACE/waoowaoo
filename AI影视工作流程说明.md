...
| 项目 | 内容 |
|------|------|
| **缺失功能** | 目标受众/平台预配置；全局视觉色调规范；项目模板库；内容风格基调锁定 |
| **优先级** | 🟡 中 |
| **待实现** | ① **平台导出格式预设**：在 videoRatio 之外新增"目标平台"字段（抖音 9:16 / B站 16:9 / YouTube 横屏），后续各阶段按此生成对应尺寸的分镜图和视频 ② **全局色调基调**：新增 `colorGradePreset` 配置（赛博朋克蓝冷 / 暖黄民国风 / 黑暗童话紫），在阶段 6 生成 prompt 时注入色调关键词，统一全集画面色感 ③ **项目模板库**：预存"古装言情""都市甜宠""悬疑犯罪"等行业模板，一键填充 artStyle + contentTrack + workflowProfile 组合配置 |
| **实现好处** | 减少配置决策成本；全集色调统一从源头保证；新用户上手时间从 30 分钟压缩到 5 分钟 |
