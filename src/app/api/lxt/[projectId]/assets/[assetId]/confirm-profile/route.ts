...
export const POST = apiHandler(async (
  ...
  const artStyleName = typeof body?.artStyle === 'string' ? body.artStyle.trim() : undefined
...
function buildDescriptionPrompt(
  ...
  if (kind === 'character' && profileData) {
    ...
    const styleInstruction = artStyleName
      ? `严格使用【${artStyleName}】风格生成描述，禁止自行推断或改变风格`
      : '风格类型从Q版卡通/半写实/写实/二次元/水墨风/3D渲染中自选最匹配的'

    const sectionOrder = isNonHuman
      ? [
        `① 风格锚定+物种+体型概述：以"[风格类型]，一[年龄][性别][物种]"开头（${styleInstruction}），跟上头身比+体型概述词（如"3头身Q版比例，体型丰满圆润"）`,
        '② 服装：逐件展开每一件衣物（上衣到内搭到下装或连体）：穿法 + 颜色 + 材质 + 款式细节',
        '③ 默认表情+面部：默认表情描述（如"自然微笑""平静温和"等）+ 物种面部类型描述 + 各部位（吻部/鼻孔/脸颊/下巴等）的颜色形态',
      ...
      : [
        `① 风格锚定+角色定位：[风格类型] + 角色名 + 年龄性别类型（${styleInstruction}；年龄如"约二十五岁男性"）`,
        '② 默认表情+面部：默认自然表情（如"平静温和""微笑"等）+ 脸型 + 眉形 + 眼部 + 鼻型 + 嘴唇 + 肤质质感',
        '③ 辨识特征：若有辨识标志字段，此处写出',
        '④ 头发：颜色 + 长度 + 发型样式 + 刘海类型',
        '⑤ 头身比+体型：头身比数据（如"7头身标准成人比例"或"5头身少年比例"）+ 身形 + 肩宽 + 高挑/娇小感' + (profileData.body_proportion ? '（profileData已含头身比参考）' : ''),
        '⑥ 服装：上衣外层（颜色+材质+领型+款式）→ 内搭（颜色+材质+款式）→ 下装（颜色+材质+款式）',
        '⑦ 鞋履：颜色 + 鞋款类型',
      ].join('\n   ')

    const exampleNonHuman = 'Q版卡通风格, 一只成年雌性家猪, 3头身Q版比例, 体型丰满圆润, , 穿着一件米白色棉质圆领长袖上衣, 外层围蓝底白格带碎花棉质围裙, 系带在背后打结覆盖躯干前部, , 自然微笑表情, 典型猪科动物面部, 吻部突出粉红色, 鼻孔大而圆, 脸颊肉质饱满, , 站立高度约120厘米, 体态宽大厚实, 腹部圆润, 四肢粗短有力, 皮肤呈粉红色, 表面有少量稀疏白色刚毛, , 眼睛小且黑亮, 位于头部两侧上方, 眼睑呈浅粉色, 耳朵大而下垂紧贴脸颊, 吻部湿润有光泽, 嘴角自然闭合, , 无人类头发, 头部覆盖稀疏浅粉色短鬃毛, 耳后鬃毛稍长, , 无独立下装, 围裙下摆延伸至膝盖下方, 露出粉红色猪蹄, , 颈部佩戴细款红色布质项圈, 无任何挂饰, , '
    const exampleHuman = '半写实风格, 约二十五岁男性, 7头身标准成人比例, , 平静温和表情, 瓜子脸轮廓清晰, 剑眉平直, 双眼皮杏形眼, 高挺鼻梁, 薄唇线条利落, 细腻肤质, , 眼角一颗小巧泪痣, , 黑色短发利落后梳, 无刘海, , 身形高挑健硕, 肩宽背阔, , 深蓝色锦缎立领长袍, 内搭米白色薄绸内衬, 下身配同色系宽腿长裤, , 黑色皮质长靴, , 腰系玉带一根, , '
    ...
    const parts: string[] = [
      ...
      '   ' + sectionOrder,
      '',
      `3. 第一区必须以风格类型开头（${styleInstruction}），确保 AI 生图风格统一`,
      '4. 默认表情必须在面部描述开头明确写出（如"自然微笑""平静温和""开心活泼"等），避免生图表情空洞',
      '5. 每个属性词必须尽量带：颜色词 + 材质/质感词 + 形态描述词（三者尽量都有）',
      '6. 禁止写：性格、心理、行为、情感、故事背景、评级说明、推理过程、标题、序号',
      '7. 直接输出描述文本，不加任何标题或括号注释',
      ...(isNonHuman
        ? [
          '8. ⚠️ 非人类角色强制规则：必须输出全部 8 个区域（①物种概述 ②服装 ③表情+面部 ④体态+测量 ⑤眼耳口 ⑥头部毛发 ⑦下身+脚蹄 ⑧配饰），不可省略任何区域。⑦下身+脚蹄 必须包含脚蹄形态/尾巴/下摆延伸/是否露蹄。⑧配饰若无可写"无配饰"但区域不可省略。',
        ]
        : []),
      '',
      '【参考格式示例（仅示意分区，不要照抄）】',
      ...
      '',
      ...(isNonHuman
        ? ['输出前自检：确认 8 个区域（①物种概述 ②服装 ③表情+面部 ④体态+测量 ⑤眼耳口 ⑥头部毛发 ⑦下身+脚蹄 ⑧配饰）全部有内容，无空双逗号段落，然后输出。']
        : []),
      '输出结果（仅描述文本，不含其他内容）：',
    ]
  ...
  if (kind === 'character') {
    ...
    const order = isNonHuman
      ? '风格锚定+物种+体型概述（头身比）→ 服装（逐件展开）→ 默认表情+面部 → 体态+皮肤 → 眼耳口 → 头部毛发 → 下身+脚蹄 → 配饰'
      : '风格锚定+角色定位 → 默认表情+面部（脸型+眉+眼+鼻+嘴+肤）→ 发型 → 头身比+体型 → 服装（上衣到内搭到下装）→ 鞋履 → 配饰'
    const parts: string[] = [
      ...
      '2. 覆盖区域顺序：' + order,
      `3. 第一区必须以风格类型开头（${styleInstruction}）`,
      '4. 默认表情必须在面部描述开头明确写出（如"自然微笑""平静温和"等）',
      '5. 每个属性词尽量带颜色词 + 材质/质感词 + 形态描述词',
      '6. 禁止写：性格、心理、行为、情感、故事背景',
      '7. 直接输出描述文本，不加任何标题或序号',
      '',
      '输出结果（仅描述文本）：',
    ]
    return parts.filter((p) => p !== null).join('\n')
