...
export type DirectorModeRunParams = {
  episodeId: string
  content: string
  model?: string
  temperature?: number
...
export type DirectorModeRunResult = RunResult

type UseDirectorModeRunStreamOptions = {
  projectId: string
  episodeId?: string | null
}

export function useDirectorModeRunStream({ projectId, episodeId }: UseDirectorModeRunStreamOptions) {
  return useRunStreamState<DirectorModeRunParams>({
    projectId,
    endpoint: (pid) => `/api/novel-promotion/${pid}/director-mode-stream`,
    storageKeyPrefix: 'novel-promotion:director-mode-run',
