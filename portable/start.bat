...
SET "APP_WORKER=%ROOT%\app\server\worker.js"
...
PowerShell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$lines=@('// Auto-generated by start.bat - do not edit manually','// Forces synchronous stdout/stderr via fs.writeSync so log lines appear immediately in portable mode','var fs=require(''fs'');','var _log=console.log;var _err=console.error;','console.log=function(){var t=Array.prototype.slice.call(arguments).join('' '')+(''\n'');try{fs.writeSync(1,t,''utf8'')}catch(e){_log.apply(console,arguments)}};','console.error=function(){var t=Array.prototype.slice.call(arguments).join('' '')+(''\n'');try{fs.writeSync(2,t,''utf8'')}catch(e){_err.apply(console,arguments)}};',('require('+\"'./worker.js'\"+')'+ ';')); [System.IO.File]::WriteAllText($env:_WORKER_SHIM, ($lines -join \"`n\"), [System.Text.Encoding]::UTF8)"
