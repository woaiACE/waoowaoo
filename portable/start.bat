...
REM WAOOWAOO_LOG_DIR: 将结构化 app 日志（file-writer 写入的项目/全局日志）路由到用户可见的日志目录
REM 与便携版控制台尾部读取的目录一致，避免日志写到只读的 app\server\ 子目录内
SET WAOOWAOO_LOG_DIR=%LOGS_DIR%
...
REM ---- Create Next.js server startup shim (same stdout-flush fix as worker shim) ----
REM  Next.js standalone server.js is CJS; we load it via require() after patching console.
SET "_NEXTJS_SHIM=%ROOT%\app\server\_nextjs_start.js"
PowerShell -NoProfile -ExecutionPolicy Bypass -Command ^
    "$lines=@('// Auto-generated by start.bat - do not edit manually','// Forces synchronous stdout/stderr via fs.writeSync so log lines appear immediately in portable mode','var fs=require(''fs'');','var _log=console.log;var _err=console.error;','console.log=function(){var t=Array.prototype.slice.call(arguments).join('' '')+(''\'n'');try{fs.writeSync(1,t,''utf8'')}catch(e){_log.apply(console,arguments)}};','console.error=function(){var t=Array.prototype.slice.call(arguments).join('' '')+(''\'n'');try{fs.writeSync(2,t,''utf8'')}catch(e){_err.apply(console,arguments)}};','require(''./server.js'');'); [System.IO.File]::WriteAllText($env:_NEXTJS_SHIM, ($lines -join "`n"), [System.Text.Encoding]::UTF8)"
